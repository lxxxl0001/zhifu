...
var firstTopDisLayer = cc.Layer.extend({
	...
	firstBtmDisLayer = cc.Layer.extend({
		...
		init: function() {
			...
			this._curQihao = cc.LabelTTF.create(" ", "Arial", 24);
			this.setNumberText(this._curQihao, cc.p(-174, -25));
		...
		setCurData: function(a) {
			...
			11 < b ? this._curDaXiao.setString("\u5927") : this._curDaXiao.setString("\u5c0f");
			0 === b % 2 ? this._curDanShuang.setString("\u53cc") : this._curDanShuang.setString("\u5355");
	...
	pk10 = cc.Layer.extend({
		...
		init: function() {
			...
			$.getJSON(g_hostIp,{t:Math.random()},function(b) {
				...
				a._btmLayer._curQihao.setString(b.current.periodNumber, cc.p(-174, -25), 24);
		...
		loadDataFromServer: function(a) {
			var b = this;
			$.getJSON(g_hostIp,{t:Math.random()},function(a) {
				if (b._nextPed == a.current.periodNumber) {
					...
					b._btmLayer._curQihao.setString(a.current.periodNumber, cc.p(-174, -25), 24);
	...
	result = cc.Layer.extend({
		...
		initByParas: function(a, b) {
			...
			this.btmLayer._curQihao.setString(a.periodNumber, cc.p(-174, -25), 24)
		},
		init: function() {
			var a = this;
	...
	MidActionLayer = cc.Layer.extend({
		...
		initCarSprite: function(a) {
			...
			b = cc.Sprite.create("assets/res/rank3.png");
			...
			b = cc.Sprite.create("assets/res/rank2.png");
			b.setPosition(cc.p(120, 350));
			...
			a = cc.Sprite.create("assets/res/rank1.png");
...
var ParticleDemo = cc.LayerGradient.extend({
	...
	DemoFlower = ParticleDemo.extend({
		onEnter: function() {
			...
			a = cc.TextureCache.getInstance().addImage("assets/res/ffff.png");
